...
with open('imdb-movies-text.txt', 'w') as out_file:
    with open('imdb-movies.csv', 'r') as in_file:
        for i, line in enumerate(in_file):
            title = re.sub(r'[^\w\s]', '', line.split(',')[1].strip())
            if i != 0:
...
if os.path.isdir("model"):
    print("From local folder...")
    embed_fn = hub.load("model")
else:
    print("From TF-Hub...")
...
@app.get("/q")
def read_item(query: str, n: Optional[int] = 5):
